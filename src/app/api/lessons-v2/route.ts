...
export async function POST(request: Request) {
  try {
    ...
    if (!title || !slug || !course_id || lesson_order === undefined) {
      return NextResponse.json(
        { error: "Title, slug, course_id, and lesson_order are required" },
        { status: 400 }
      );
    }
    ...
    if (
      !profile ||
      (profile.role !== "admin" && course.instructor_id !== user.id)
    ) {
      return NextResponse.json(
  ...
  try {
    ...
    const { data: lesson } = await supabase
      ...
      .select(
        `
        course_id,
        courses!courses_lessons_course_id_fkey (
          instructor_id
        )
      `
      )
    ...
    if (
      !profile ||
      (profile.role !== "admin" && lesson.courses[0]?.instructor_id !== user.id)
    ) {
      return NextResponse.json(
