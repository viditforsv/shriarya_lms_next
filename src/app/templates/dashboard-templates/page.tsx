'use client'

import Link from 'next/link'
import { Button } from '@/components/ui/button'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { ArrowLeft, ArrowRight } from 'lucide-react'

export default function DashboardTemplatesIndex() {
  const dashboardTemplates = [
    // Phase 1 (MVP - Launch) - Basic Dashboards
    {
      name: "Student Dashboard (basic)",
      href: "/templates/dashboard-templates/student-dashboard",
      phase: "Phase 1 - MVP"
    },
    {
      name: "Instructor Dashboard (basic)",
      href: "/templates/dashboard-templates/instructor-dashboard",
      phase: "Phase 1 - MVP"
    },
    
    // Phase 2 (Growth) - Enhanced Dashboards
    {
      name: "Student Dashboard (enhanced)",
      href: "/templates/dashboard-templates/student-dashboard",
      phase: "Phase 2 - Growth"
    },
    {
      name: "Instructor Dashboard (enhanced)",
      href: "/templates/dashboard-templates/instructor-dashboard",
      phase: "Phase 2 - Growth"
    },
    {
      name: "Analytics (basic)",
      href: "/templates/dashboard-templates/analytics",
      phase: "Phase 2 - Growth"
    },
    
    // Phase 3 (Scale) - Advanced Dashboards
    {
      name: "Admin Dashboard (advanced)",
      href: "/templates/dashboard-templates/admin-panel",
      phase: "Phase 3 - Scale"
    },
    {
      name: "Institution Analytics",
      href: "/templates/dashboard-templates/institution-dashboard",
      phase: "Phase 3 - Scale"
    },
    {
      name: "Teacher Signup",
      href: "/templates/dashboard-templates/teacher-signup",
      phase: "Phase 3 - Scale"
    }
  ]

  // Group templates by phase
  const groupedTemplates = {
    "Phase 1 - MVP": dashboardTemplates.filter(t => t.phase === "Phase 1 - MVP"),
    "Phase 2 - Growth": dashboardTemplates.filter(t => t.phase === "Phase 2 - Growth"),
    "Phase 3 - Scale": dashboardTemplates.filter(t => t.phase === "Phase 3 - Scale")
  }

  return (
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8">
        {/* Breadcrumbs */}
        <div className="mb-8">
          <Link 
            href="/templates" 
            className="inline-flex items-center text-muted-foreground hover:text-foreground transition-colors"
          >
            <ArrowLeft className="w-4 h-4 mr-2" />
            Back to Templates
          </Link>
        </div>

        {/* Header */}
        <div className="mb-12">
          <h1 className="text-3xl font-bold text-[#1e293b] mb-2">Dashboard Templates</h1>
          <p className="text-muted-foreground">
            Professional dashboard templates organized by development phases for different user roles
          </p>
        </div>

        {/* Templates by Phase */}
        {Object.entries(groupedTemplates).map(([phase, templates]) => (
          <div key={phase} className="mb-12">
            <h2 className="text-xl font-semibold text-[#1e293b] mb-4 border-b border-[#feefea] pb-2">
              {phase}
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {templates.map((template, index) => (
                <Card key={index} className="hover:shadow-md transition-all duration-200 border-[#feefea] hover:border-[#e27447]">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-base text-[#1e293b] text-center">{template.name}</CardTitle>
                  </CardHeader>
                  <CardContent className="pt-0 pb-4">
                    <Link href={template.href}>
                      <Button 
                        size="sm" 
                        variant="outline" 
                        className="w-full border-[#1e293b] hover:bg-[#1e293b] hover:text-white transition-colors text-xs"
                      >
                        View
                        <ArrowRight className="w-3 h-3 ml-1" />
                      </Button>
                    </Link>
                  </CardContent>
                </Card>
              ))}
            </div>
          </div>
        ))}

        {/* Quick Stats */}
        <div className="mt-12 text-center text-sm text-muted-foreground">
          {dashboardTemplates.length} dashboard templates available across {Object.keys(groupedTemplates).length} phases
        </div>
      </div>
    </div>
  )
}
