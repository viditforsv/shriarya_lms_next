...
export function useCourseData(courseSlug: string) {
  ...
  const determineLessonType = (title: string, content: string | null): 'video' | 'document' | 'question' | 'practice' => {
    const titleLower = title.toLowerCase()
    const contentLower = content?.toLowerCase() || ''

    if (titleLower.includes('practice') || titleLower.includes('problem') || titleLower.includes('exercise')) {
      return 'practice'
    } else if (titleLower.includes('question') || titleLower.includes('quiz') || titleLower.includes('test')) {
      return 'question'
    } else if (contentLower.includes('video') || titleLower.includes('video') || titleLower.includes('lecture')) {
      return 'video'
    } else {
      return 'document'
    }
