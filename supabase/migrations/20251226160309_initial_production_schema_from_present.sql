-- Initial Production Schema Migration from present_schema.sql
-- This migration creates all tables for the production database
-- Run this via Supabase CLI: supabase db push
--
-- Table order is carefully arranged to respect foreign key dependencies
...
-- Courses Templates (depends on auth.users)
...
-- GRE Questions (depends on auth.users)
...
-- ============================================================================
-- DEPENDENT TABLES (Depend on base tables)
-- ============================================================================
...
-- Courses Chapters (depends on courses_units and courses)
...
-- Courses Topics (depends on courses_chapters and courses)
...
-- Quizzes (depends on courses_lessons, but lesson_id is nullable so we create it before lessons)
-- Note: The foreign key constraint will be added after courses_lessons is created
...
-- Courses Lessons (depends on courses, quizzes, courses_chapters, courses_topics)
...
-- Add foreign key constraint for quizzes.lesson_id now that courses_lessons exists
-- Using DO block to safely add constraint only if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.quizzes 
      ...
      FOREIGN KEY (lesson_id) REFERENCES public.courses_lessons(id);
  END IF;
END $$;
...
-- User Progress (depends on auth.users and courses)
...
CREATE TABLE IF NOT EXISTS public.qa_questions (
  ...
  overall_rating numeric GENERATED ALWAYS AS (
    CASE 
      WHEN content_accuracy IS NULL AND difficulty_appropriateness IS NULL 
           AND clarity_rating IS NULL AND solution_quality IS NULL 
      THEN NULL
      ELSE (((COALESCE(content_accuracy, 0) + COALESCE(difficulty_appropriateness, 0) + 
              COALESCE(clarity_rating, 0) + COALESCE(solution_quality, 0)))::numeric / 4.0)
    END
  ) STORED,
...
-- QA History (depends on auth.users)
...
-- Assignment Submissions (depends on auth.users and courses)
...
-- Lesson Feedback (depends on auth.users)
